...
vols = [sum(p[2] .>= 0.5) for p in b0.particles]
mean_vols = mean(vols)
std_vols = std(vols)
...
[push!(s0.particles, p) for p in b0.particles]
println("Entering Simulation...")
for (sp, a, o, r, bp, t) in stepthrough(m, planner, up, b0, s0, "sp,a,o,r,bp,t", max_steps=50)
    ...
    empty!(sp.particles)
    [push!(sp.particles, p) for p in bp.particles]
end
